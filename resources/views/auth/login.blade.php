<x-guest-layout>
    @push('app_name') // Log In @endpush
    <form method="POST" action="{{ route('login') }}" class="login__form">
        @csrf
        <h1 class="login__title text-sm">Login</h1>
        <div class="flex flex-row justify-center space-x-4 social-login">
            <a href="auth/google/redirect"
                class="text-white bg-[#4285F4] hover:bg-[#4285F4]/90 focus:ring-4 focus:outline-none focus:ring-[#4285F4]/50 font-medium rounded-lg text-sm px-5 py-2.5 text-center inline-flex items-center dark:focus:ring-[#4285F4]/55 me-2 mb-2 w-full">
        ...
        @if ($errors->has('email'))
            <span class="text-sm text-red-800 mb-2">Please check again; something went wrong.
        ...
        <div class="login__inputs">
            <div class="login__box">
                <input type="email" name="email" value="{{ old('email') }}" placeholder="Email ID"
        ...
        <div class="login__check">
            <div class="login__check-box">
                <input type="checkbox" class="login__check-input" name="remember" id="user-check">
